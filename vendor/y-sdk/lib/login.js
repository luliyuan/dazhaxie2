/**
 * Author：yangjianmin
 *参考自腾讯云小程序demo
 */
var utils = require('./utils');
var constants = require('./constants');
var Session = require('./session');
/***
 * @class
 * 表示登录过程中发生的异常
 */

/***
 * @class
 * 表示登录过程中发生的异常
 */

var LoginError = (function () {
  function LoginError(type, message) {
    Error.call(this, message);
    this.type = type;
    this.message = message;
  }

  LoginError.prototype = new Error();
  LoginError.prototype.constructor = LoginError;

  return LoginError;
})();

var noop = function noop() { };
var defaultOptions = {
  method: 'GET',
  success: noop,
  fail: noop,
  loginUrl: null,
  userInfoUrl: null
};
/**
 * 微信登录，获取 code 和 encryptData
 */console.log("11")
var getWxLoginResult = function getLoginCode(callback) {
	console.log("dddd")
  wx.login({
    success: function (loginResult) {
			console.log('loginResult1',loginResult)
      callback(null, {
        code: loginResult.code,
        // encryptedData: userResult.encryptedData,
        // iv: userResult.iv,
        // userInfo: userResult.userInfo,
      });
      // wx.getUserInfo({
      //   success: function (userResult) {
      //     callback(null, {
      //       code: loginResult.code,
      //       encryptedData: userResult.encryptedData,
      //       iv: userResult.iv,
      //       userInfo: userResult.userInfo,
      //     });
      //   },

      //   fail: function (userError) {
      //     var error = new LoginError(constants.ERR_WX_GET_USER_INFO, '获取微信用户信息失败，请检查网络状态');
      //     error.detail = userError;
      //     callback(error, null);
      //   },
      // });
    },

    fail: function (loginError) {
			console.log("loginfail")
      var error = new LoginError(constants.ERR_WX_LOGIN_FAILED, '微信登录失败，请检查网络状态');
      error.detail = loginError;
      callback(error, null);
    },
  });
};
/**
 * @method
 * 进行服务器登录，以获得登录会话
 *
 * @param {Object} options 登录配置
 * @param {string} options.loginUrl 登录使用的 URL，服务器应该在这个 URL 上处理登录请求
 * @param {string} [options.method] 请求使用的 HTTP 方法，默认为 "GET"
 * @param {Function} options.success(userInfo) 登录成功后的回调函数，参数 userInfo 微信用户信息
 * @param {Function} options.fail(error) 登录失败后的回调函数，参数 error 错误信息
 */
var login = function login(options) {
  options = utils.extend({}, defaultOptions, options);

  if (!defaultOptions.loginUrl) {
    options.fail(new LoginError(constants.ERR_INVALID_PARAMS, '登录错误：缺少登录地址，请通过 setLoginUrl() 方法设置登录地址'));
    return;
  }

  var doLogin = () => getWxLoginResult(function (wxLoginError, wxLoginResult) {
    if (wxLoginError) {
      options.fail(wxLoginError);
      return;
    }

    // var userInfo = wxLoginResult.userInfo;

    // 构造请求头，包含 code、encryptedData 和 iv
    var code = wxLoginResult.code;
    // var encryptedData = wxLoginResult.encryptedData;
    // var iv = wxLoginResult.iv;
    var ysession = {}

    // 请求服务器登录地址，获得会话信息
    wx.request({
      url: options.loginUrl,

      data: {
        code: code,
        // encryptedData: encryptedData,
        // iv: iv
      },

      success: function (result) {
				console.log("21111111111111111111111")
				console.log("dd111dd",result)
        var data = result.data;
        // console.log('ssss,result')
        // 成功地响应会话信息
				console.log(data)
        if (data.msg == 0) {
          if (data.openId) {
             console.log('csadssssssssssssssssssssssssssssssssssss',data)
            ysession = data
            ysession.openId = data.openId;
// 							wx.request({
// 								url: options.userInfoUrl,
// 								data: {
// 									userName: result.data.user.name,
// 									face: result.data.user.face,
// 									sex: result.data.user.sex,
// 									openId: result.data.openId,
// 									userId: result.data.userId
// 								},
// 								success: function (resData) {
// 									console.log(resData)
// 									if (resData.data.msg == 0) {
// 									}
// 								},
// 								fail: function (loginResponseError) {
// 								
// 								},
// 							})
							
						
						
            // wx.request({
            //   url: options.userInfoUrl,
            //   data: {
            //     userName: userInfo.nickName,
            //     face: userInfo.avatarUrl,
            //     sex: userInfo.gender,
            //     openId: data.openId
            //   },
            //   success: function (resData) {
            //     if (resData.data.msg == 0) {
            //       ysession.user = resData.data.user;
            //       ysession.userId = resData.data.userId;
            //       Session.set(ysession);
            //       options.success(ysession);
            //     }

            //     // that.globalData.userId = data.data.userId;
            //     // var user = data.data.user;
            //     // that.globalData.user = user
            //     // typeof cb == "function" && cb(that.globalData.userInfo)
            //   }
            // })

            //   }
            // })

            ysession.user = data.user?data.user:'';
            ysession.userId = data.userId?data.userId:'';

            Session.set(ysession);
            options.success(ysession);
            console.log('login',data)
            if (data.user.create_time&&data.user.name){
								console.log(data.user)
              }else{
                wx.navigateTo({
                  url: '/pages/compage/start/start',
                })
              }
            // Session.set(data.session);
            // options.success(userInfo);
          } else {
            var errorMessage = '登录失败';
            var noSessionError = new LoginError(constants.ERR_LOGIN_SESSION_NOT_RECEIVED, errorMessage);
            options.fail(noSessionError);
          }

          // 没有正确响应会话信息
        } else {
          var errorMessage = '登录失败，没有正确响应会话信息';
          var noSessionError = new LoginError(constants.ERR_LOGIN_SESSION_NOT_RECEIVED, errorMessage);
          options.fail(noSessionError);
        }
      },

      // 响应错误
      fail: function (loginResponseError) {
        var error = new LoginError(constants.ERR_LOGIN_FAILED, '登录失败，可能是网络错误或者服务器发生异常');
        options.fail(error);
      },
    });
  });

  var session = Session.get();
	console.log("sdfdssdf",session)
  if (session && session.user && session.userId) {
    wx.checkSession({
      success: function () {
         console.log('checkSession success')
        options.success(session);
      },

      fail: function () {
         console.log('checkSession fail')
        Session.clear();
        doLogin();
      },
    });
  } else {
    doLogin();
  }
};
var setLoginUrl = function (loginUrl) {
  defaultOptions.loginUrl = loginUrl;

};
/* @method
 * 服务器登录，以获得登录会话
 *
 * @param {Object} options 登录配置
 * @param {string} options.loginUrl 登录使用的 URL，服务器应该在这个 URL 上处理登录请求
 * @param {string} [options.method] 请求使用的 HTTP 方法，默认为 "GET"
 * @param {Function} options.success() 登录成功后的回调函数
 * @param {Function} options.fail(error) 登录失败后的回调函数，参数 error 错误信息
 */
var requestLogin = function requestLogin(options) {

  if (!/http/.test(defaultOptions.loginUrl)) {
    options.fail(new LoginError(constants.ERR_INVALID_PARAMS, '登录错误：缺少登录地址，请通过 setLoginUrl() 方法设置登录地址'));
    return;
  }

  options = utils.extend({}, defaultOptions, options);

  // 构造请求头，包含 code、encryptedData 和 iv
  var code = options.loginParams.code;
  var encryptedData = options.loginParams.encryptedData;
  var iv = options.loginParams.iv;
  var header = {};
  var ytsession = Session.get();
  var yuser = options.loginParams.userInfo;
  ytsession.userInfo = options.loginParams.userInfo ? options.loginParams.userInfo:{};
  header[constants.WX_HEADER_CODE] = code;
  header[constants.WX_HEADER_ENCRYPTED_DATA] = encryptedData;
  header[constants.WX_HEADER_IV] = iv;

console.log('yuser',yuser)
  // 请求服务器登录地址，获得会话信息
  wx.request({
    url: options.loginUrl,
    header: header,
    method: options.method,
    data: options.data,
    success: function (result) {
      var data = result.data;
				console.log("dataaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",data)
      // 成功地响应会话信息
      if (data && data.msg == 0) {
        // var res = data.data
        wx.request({
          url: options.userInfoUrl,
          data: {
            userName: yuser.nickName,
            face: yuser.avatarUrl,
            sex: yuser.gender,
            openId: data.openId,
            userId: data.userId
          },
          success: function (resData) {
						console.log(resData)
            if (resData.data.msg == 0) {
              ytsession.user = resData.data.user;
              ytsession.userId = resData.data.userId;
              Session.set(ytsession);
              options.success(ytsession);
            }
          },
          fail: function (loginResponseError) {
            var errorMessage = '登录失败(' + data.error + ')：' + (data.message || '未知错误');
            var noSessionError = new LoginError(constants.ERR_LOGIN_SESSION_NOT_RECEIVED, errorMessage);
            options.fail(noSessionError);
          },
        })
        // if (data.msg == 0) {
        //   console.log('rlogin')
        //   Session.set(res);
        //   options.success();
        // } else {
        //   var errorMessage = '登录失败(' + data.error + ')：' + (data.message || '未知错误');
        //   var noSessionError = new LoginError(constants.ERR_LOGIN_SESSION_NOT_RECEIVED, errorMessage);
        //   options.fail(noSessionError);
        // }

        // 没有正确响应会话信息
      } else {
        var noSessionError = new LoginError(constants.ERR_LOGIN_SESSION_NOT_RECEIVED, JSON.stringify(data));
        options.fail(noSessionError);
      }
    },

    // 响应错误
    fail: function (loginResponseError) {
			console.log(loginResponseError)
      var error = new LoginError(constants.ERR_LOGIN_FAILED, '登录失败，可能是网络错误或者服务器发生异常');
      options.fail(error);
    },
  });



}
var setUserInfoUrl = function (userInfoUrl) {
  defaultOptions.userInfoUrl = userInfoUrl;

};

module.exports = {
  LoginError: LoginError,
  requestLogin: requestLogin,
  login: login,
  setLoginUrl: setLoginUrl,
  setUserInfoUrl: setUserInfoUrl
};